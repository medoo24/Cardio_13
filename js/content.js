...
window.ArrhythmiaContent = (() => {
  ...
  function revisionPage() {
    return page("revision", `
      ...
      ${sectionHeading("Flashcards", "Click or press Enter to flip each card.", "${flashcards.length} cards")}
      <section class="flashcard-grid interactive-only" id="flashcard-grid">
        ${flashcards.map(([front, back], index) => `<button class="flashcard" type="button" aria-pressed="false"><span class="flashcard-inner"><span class="flashcard-face"><strong>${front}</strong><small>Card ${index + 1} · Flip for answer</small></span><span class="flashcard-face flashcard-back"><strong>${back}</strong><small>Click to return</small></span></span></button>`).join("")}
      </section>

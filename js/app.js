...
(() => {
  ...
  function setFlashcardState(card, flipped) {
  ...
  function setupFlashcards() {
    ...
    grid.addEventListener("click", (event) => {
      ...
      setFlashcardState(card, card.getAttribute("aria-pressed") !== "true");
    });

    document.getElementById("flip-all-flashcards")?.addEventListener("click", () => {
      grid.querySelectorAll(".flashcard").forEach((card) => setFlashcardState(card, true));
    });

    document.getElementById("reset-flashcards")?.addEventListener("click", () => {
      grid.querySelectorAll(".flashcard").forEach((card) => setFlashcardState(card, false));
      grid.querySelector(".flashcard")?.focus();
    });
  }
